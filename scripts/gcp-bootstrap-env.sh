#!/usr/bin/env bash
# One-time: generate production secrets and upload backend/.env to the VM.
# Does NOT redeploy application containers.
set -euo pipefail
...
if [[ -z "$PUBLIC_URL" ]]; then
  ...
  echo "Re-run with PUBLIC_URL=https://app.yourdomain.com after DNS is ready."
else
...
echo "==> Uploading backend/.env to VM (overwrites existing)..."
gcloud compute ssh "$GCP_INSTANCE" --zone="$GCP_ZONE" --project="$GCP_PROJECT_ID" \
  --command="mkdir -p ~/da-ad-hackathon/backend"

gcloud compute scp "$ENV_FILE" "${GCP_INSTANCE}:~/da-ad-hackathon/backend/.env" \
  --zone="$GCP_ZONE" --project="$GCP_PROJECT_ID"

echo "Done. Secrets are on the VM only (not in git)."
